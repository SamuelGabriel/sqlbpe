"""From J.K. Kummerfeld, the original can be found here https://github.com/jkkummerfeld/text2sql-data/blob/master/tools/tokenise_sql.py.
"""
...
def tokenise(query):
    ...
    for token in query.split():
        ...
        if not (in_squote or in_dquote):
            m = re.search(r"(?P<table>[A-Z_]+)(?P<alias>aliasr?\d+)", token)
            if m:
